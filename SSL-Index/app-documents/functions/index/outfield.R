  ...
  indexQuery(
    paste(
      "SELECT
        ...
        `successful presses` / `attempted presses` AS `press%`,
        `successful open play crosses` / `attempted open play crosses` AS `open play crosses%`,
        `shots on target` / `shots` AS `shot accuracy%`,
    ...
    FROM (
      ...
      FROM (
        ...
        WHERE division = '", league, "' AND season = ", season,
        ") `q01`
        GROUP BY `name`
      ) `q01`",
      sep = "")
