            ...
            if(nrow(schedule) != 0){
              lapply(1:nrow(schedule), function(i) {
                ...
                fluidRow(
                  ...
                  column(2, selectInput(paste0("home_", id) %>% session$ns(), label = NULL, choices = c("", teams$team), selected = game$Home)) %>% div(class = "smallSelect"),
                  column(2, selectInput(paste0("away_", id) %>% session$ns(), label = NULL, choices = c("", teams$team), selected = game$Away)) %>% div(class = "smallSelect"),
